		...
		noisyPath(path, amplitude) {
			...
			const noise = amplitude => {
				return command => {
					if (command.type === 32) {
						command.x1 =
							command.x1 + Math.random() * amplitude;
						command.y1 =
							command.y1 + Math.random() * amplitude;
						command.x2 =
							command.x2 + Math.random() * amplitude;
						command.y2 =
							command.y2 + Math.random() * amplitude;
					}
					return command;
				};
			};
